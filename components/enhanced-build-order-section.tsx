import { getItemIcon, handleImageError } from "@/lib/riot-assets"
...
export default function EnhancedBuildOrderSection({ buildItems }: BuildOrderSectionProps) {
  return (
    <div className="p-4">
          ...
          <div className="space-y-2">
            {buildItems.early.map((item) => (
              <div key={item.id} className="flex items-center gap-2 bg-background/50 p-2 rounded-md">
                <img
                  src={getItemIcon(item.name) || "/placeholder.svg"}
                  alt={item.name}
